...
# Primary config/claude-function-hooks is a captain-wide opt-in (presence
# alone enables Claude Code's function-hooks surface for every claude
# launch), so it flows down too and a secondmate's own claude crewmates get
# the same posture.
# It also pushes
...
FM_INHERITABLE_CONFIG="${FM_INHERITABLE_CONFIG:-crew-dispatch.json crew-harness backlog-backend backend herdr-presentation-spaces startup-memory-budget trace-context launch-env-allowlist claude-permission-mode claude-function-hooks lavish-axi-host}"

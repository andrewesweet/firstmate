...
# replay can prove a recovered brief byte-identical; the snapshot is hashed, and
# hashing is required exactly like the rules digest. The model request itself
# carries only the brief's task sections (see task_sections below); brief text is
# never recorded.
...
trap 'rm -f "$BRIEF_SNAPSHOT" "$RULES" "$RESP_FILE" "$QUOTA" "$TASK_TEXT"' EXIT

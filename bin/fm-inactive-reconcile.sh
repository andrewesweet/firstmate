...
# fm-crew-state.sh, which applies the same gate: any ship done whose head
# lives only in the disposable copy reads blocked and is not a terminal
# inactive outcome - a bare pre-validation `done: {summary}` included, which
# has no legitimate producer on this fork.

...
DOD=$(fm_dod_block "$MODE" "$ID" "$DATA" "$FORGE") || exit 1
...
if [ "$FORGE" = none ]; then
  ...
  echo "scaffolded: $BRIEF (ship, mode=$MODE forge=$FORGE shape=$SHAPE; replace {TASK}, {PUBLISHED_INTENT}, and {FIRSTMATE_SPEC})"
fi

...
Every successful ship or scout spawn appends one JSON line to the home's gitignored `data/dispatch-spawns.jsonl` carrying the UTC time, task id, kind, and the harness, model, and effort the task was actually dispatched on, resolved exactly as the task record records them; an unwritable log prints one stderr line and never fails the spawn.
`bin/fm-dispatch-resolve-score.sh` replays the two logs offline, with an optional adjudicated labels file, reporting clear-only rule and profile agreement, override rate, coverage, and the precision-versus-abstention curve across confidence floors, so the fixed 0.6 floor can be judged from local evidence before it is changed; the script's header owns its exact join and metric definitions, and live selection is unchanged.

...
`config/classifier-shadow` set to exactly `jev` joins the home's granted wakes to the shadow advisory trial: a detached Jev answer per ablation variant, recorded in `state/branch-mod-shadow.jsonl` and scored by `bin/fm-branch-shadow-score.sh`, that never delays or alters the wake path; absent or any other value is off, and [docs/claude-supervision-branch.md](claude-supervision-branch.md) "Shadow advisory trial" owns the contract.

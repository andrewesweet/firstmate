...
| `fm-brief.sh`            | Scaffold ship (explicit `--mode`), scout, secondmate-charter, and Herdr-lab briefs, with Captain's intent, Published intent, and Firstmate spec subsections on ship and Captain's intent and Firstmate spec on scout |
...
| `fm-trace-span-lib.sh`   | OTLP/HTTP lifecycle span emitter; its header owns the interface and delivery contract |
...
| `fm-branch-shadow-pane.sh` | Print the bounded, read-only pane tail, busy-state observation, window identity, and watcher stale-series markers for one task as the shadow advisory's pane evidence |
| `fm-branch-shadow-score.sh` | Score the supervision-branch shadow advisory log (both hosts) against the branch outcome store by durable wake key |
| `fm-branch-shadow-gates.sh` | Score the six candidate absorb, suppress, and escalate gates over the shadow advisory records' facts against joined outcomes, backstop surfacing, and derivable main actions, with a floor sweep and an evidence-sufficiency mode (`--sufficient <gates> --bound <p>` exits 0 only when every named gate's readable-truth fires put its 95% loss-class wrong-fire upper bound at or under the bound with enough positive-truth wakes) |
| `fm-branch-shadow-sufficient-notify.sh` | Append exactly one durable `check` wake naming the shadow gates a sufficiency watch found evaluable; the deterministic action half of an armed `fm-procevent-when.sh` sufficiency watch ([claude-supervision-branch.md](claude-supervision-branch.md)) |

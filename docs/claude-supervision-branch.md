...
Its cost section meters the trial from the same records: per variant the record count, records with usage, input-token total and median, output-token total, and latency p50/p95 over every record (unavailable calls still cost time), plus one summed line per granted wake key.

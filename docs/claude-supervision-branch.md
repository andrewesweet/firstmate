...
The first three are portable (Node and bash; the eligibility test drives the mod's exported `bind` and `scopeForUnreadWake`, see [`pi-supervision-branch.md`](pi-supervision-branch.md) for the three-fold guarantee it pins), the fourth runs `claude plugin validate --strict` and the engine-hosted `claude plugin test` suite wherever `claude` is installed, and the live test submits a few Sonnet turns in a temporary scratch home and skips unless `claude --version` is exactly the pin and `tmux` exists; it also proves, across two labs, that a minutes-later wake resumes the persisted agent when transcript saving is on and rotates with `why=unresumable` when an inherited `CLAUDE_CODE_CHILD_SESSION` breaks resume.

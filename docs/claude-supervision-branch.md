...
The evidence gather always runs (`bin/fm-wake-evidence.sh <task>`: the task's current state, the status lines appended since the last classified wake marked NEW, and a few earlier lines marked HISTORY), because it advances the classifier offset and feeds the covering-outcome rule whether or not any model is asked.
When every NEW line in every gathered bundle carries a recognised verb - the terminal `done|needs-decision|blocked|failed` and nonterminal `working|resolved|captain-held|paused` verbs of `bin/fm-classify-lib.sh`, matched verb-only so the legacy free-text fallback never decides a short-circuit - the whitelist verdict is emitted with no model call (`captain` when any line's verb is terminal, `routine` otherwise) and the record's `model` is `null`.
Any unrecognised line keeps the model path: every `note:` line, a bare legacy token, a failed or unparseable gather, a NEW block the gatherer marked truncated (when the 6,000-byte cap leaves status lines out of the bundle, `bin/fm-wake-evidence.sh` emits an explicit `## the NEW block above stops at the 6000-byte cap ...` section line before the HISTORY marker, on every platform, and the route refuses whenever that line is present), and a wake with zero new bytes are all evidence a model must judge, exactly as it always has.
Otherwise it is one `$.model.complete` call on the model named by `config/classifier-model` (default `haiku` on this host; [configuration.md](configuration.md) "Claude Code supervision branch" owns the resolution and fallback rule), with no thinking and `maxTokens` bounded at 200, over the wake's reason line and the gathered bundle.
...
Every classification, model call or deterministic verb route, appends one record to `state/branch-mod-classifications.jsonl`: the wake text, the tasks and queue sequences, the evidence per task (`{"task","from","to","text","text_len"}` - the byte range as always, plus a bounded copy of the bundle that was judged with the length judged, middle-truncated to the first and last half of 8,192 characters past that cap), the record-level `text_cap` naming that bound, the verdict, the reason, the model name (`null` on a verb route, whose answer is empty), the elapsed milliseconds, and the model's answer.
...
The answer call is `bin/fm-branch-shadow-jev.sh`: the mod writes the request JSON to its stdin, the helper reads `TYPESAFE_API_KEY` from the environment or the home's `.env`, passes it to `curl` only as an Authorization header on file descriptor 3 (never on argv or in the child environment), and prints one JSON line - the model, answers, and usage, or `{"ok":false,"unavailable":"<cause>"}` - and always exits 0.
...
Its cost section meters the trial from the same records: per variant the record count, records with usage, input-token total, output-token total, and latency p50/p95 over every record (unavailable calls still cost time), plus one summed line per granted wake key (records without a wake key count per variant but not per wake).
...
The opt-ins are mutually exclusive by construction, with the host stepping aside: when `state/.branch-mod-mode` is present, `bin/fm-supervision-host.sh` execs the plain watcher arm even though `config/supervision-host` opts in, so the home's supervision is exactly what the owner arms without the host, and it prints one `supervision-host:` notice naming the conflict - once per episode, suppressed by a marker until a later host run finds no mod, so a renewed conflict is surfaced again (`bin/fm-supervision-host.sh`'s header owns the mechanics).
...
- `state/branch-mod-classifications.jsonl`: the classification log, same rotation; each record carries a bounded copy of the evidence text beside its byte range so classifications stay scorable after teardown; read by `bin/fm-branch-classifier-score.sh`.

...
The same script vendors the report/processed decision core (`lib/fm-branch-report-sequence.ts`: report validation, scope verdicts, settlement append and mark-read argv and classification, and the machine-owned failure and success wordings this host renders byte-identically to Pi) and the provider-error latch machine (`lib/fm-branch-provider-latch.ts`), with this host declaring its latch policy (threshold two, fixed five-minute cooldown, no recovery probe - the doubled-value cap makes the exponential base unobservable here) and keeping its own seams: the normal-result scoping refusal, the duplicate-report guard, and the captain-facing latch strings.

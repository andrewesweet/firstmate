...
Any unrecognised line keeps the model path: every `note:` line, a bare legacy token, a failed or unparseable gather, a NEW block the gatherer cut mid-line (its 6,000-byte cap leaves the HISTORY marker fused onto the partial line, the detectable signature the route refuses), and a wake with zero new bytes are all evidence a model must judge, exactly as it always has.

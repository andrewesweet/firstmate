...
The Claude Code branch is the `fm-branch-mod` plugin under `.claude/mods/fm-branch-mod`: one function-hooks module (`hooks/branch.ts`), the canonical shared modules it delegates to (`lib/fm-branch-eligibility.ts`, `lib/fm-branch-report-sequence.ts`, and `lib/fm-branch-provider-latch.ts`), one agent definition (`agents/fm-branch.md`), and the classifier's system prompt (`classifier-system.txt`).

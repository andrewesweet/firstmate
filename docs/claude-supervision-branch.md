...
Any unrecognised line keeps the model path: every `note:` line, a bare legacy token, a failed or unparseable gather, and a wake with zero new bytes are all evidence a model must judge, exactly as it always has.

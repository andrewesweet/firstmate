...
The gate set is `absorb-no-new-outcome`, `absorb-routine-working`, `stale-active-suppress`, `pr-ready-arm`, `severity-alert`, and `candidate-order` (ordering only, no floor); a record joins ground truth from the outcome rows by wake key, from the wake-drain backstop's surfacing of uncovered captain lines, and from derivable main actions (a new worker incarnation after a stale wake, or a recorded PR behind a ready phase), and a record without facts is unscorable for a gate rather than guessed.

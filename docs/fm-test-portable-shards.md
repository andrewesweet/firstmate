...
The three `tests/fm-branch-claude-mod*.test.sh` hints and the `tests/fm-branch-mod-bin.test.sh` hint are local measurements from 2026-09-16 doubled, pending their first green CI artifacts.
Sixteen further hints for previously unmeasured deterministic serial scripts (`tests/fm-backlog-read-bound.test.sh`, `tests/fm-calm-claude-mod*.test.sh`, `tests/fm-claude-trust.test.sh`, `tests/fm-harness-precedence.test.sh`, `tests/fm-mail*.test.sh`, `tests/fm-pr-reviewers.test.sh`, `tests/fm-pr-state.test.sh`, `tests/fm-precompact-skills.test.sh`, `tests/fm-promote.test.sh`, `tests/fm-rovo-harness.test.sh`, `tests/fm-send-agy-confirm.test.sh`, `tests/fm-stat-shadowing.test.sh`, `tests/fm-tasks-axi.test.sh`, and `tests/fm-trace-span-lib.test.sh`) are local measurements from 2026-09-16 doubled for the same reason, added when the merged serial lane's unmeasured share first crossed the guard's bound.

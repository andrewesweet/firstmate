...
- `tests/fm-send-resolve-key.test.sh` (21 assertions, 1 new): a delivered answer posts its span before later decision-close bookkeeping can fail, a two-key answer carries both keys comma-joined without answer text, and a mistyped key refuses before delivery and posts no span.

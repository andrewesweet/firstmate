...
- `firstmate.wake` - `bin/fm-wake-drain.sh`, after the acknowledgement commit, per consumed queue row whose key maps to a home task through `fm_wake_status_key_map`, covering the row's queue time through the one acknowledgement instant shared by the whole batch, with `firstmate.wake.kind`, `firstmate.wake.seq`, and `firstmate.wake.key`; heartbeats, per-poll check keys, and window-keyed stale rows emit nothing. Only signal rows carry task status keys today, so `signal` is the only kind that reaches emission; the kind allowlist and the key mapping remain separate bounded checks. Presentation captures the minimum trace context of each presented task-keyed row as `state/.wake-trace.<task>` so a task legitimately torn down before the acknowledgement still gets its span; the capture is telemetry only, discarded at acknowledgement, and every failure to write or remove it is silent. One awk pass partitions the queue into retained and acknowledged rows, and no telemetry-only failure can block the acknowledgement.

`tests/fm-captain-hold-lifecycle.test.sh` grows `test_close_paths_emit_hold_spans_over_the_recorded_hold_time` (51 `ok -` lines total): it drives the real hold, answer, `--release`, and `reconcile close` entrypoints against a real tasks-axi backend with a recording fake curl, asserting one span per close joined to the held task's own trace from the hold-set stamp with the close mode and reason, the idempotent answer replay posting nothing, an `--origin`-linked call joining the origin task's trace at its own hold-set time, a taskless captain call and a refused answer posting nothing, a disabled session posting nothing, and the emitted reason bounded at 160 characters.
...
`tests/fm-wake-queue.test.sh` grows `test_acknowledgement_emits_task_keyed_wake_spans_only` (43 `ok -` lines total): one signal row for a traced task beside a window-keyed stale row, a taskless `startup-network` check row, and a heartbeat row; the acknowledgement posts exactly one `firstmate.wake` span parented on the task carrier starting at the row's queue time with kind, key, and seq, proving heartbeats and per-poll activity never emit. `test_wake_spans_survive_teardown_and_share_one_acknowledgement_instant` removes a presented task's meta before the acknowledgement and asserts its rows still post spans on the captured trace, every span of the batch shares one end instant, the capture is discarded, an unwritable capture blocks nothing and writes nothing to the presentation's stderr, and an already-acknowledged replay posts nothing.
...
The teardown suite `tests/fm-teardown.test.sh` (91 assertions) gains the root-link payload: a meta recording `trace_link=` emits the task root with exactly one span link referencing the recorded ids and parentage unchanged, an invalid recorded link emits no `links` field, and every pre-existing root case now also pins that no `trace_link=` means no links.
...
The delegated-handoff suite `tests/fm-backlog-handoff.test.sh` (27 assertions) proves the `firstmate.handoff` span contract on the local route: a two-key handoff posts exactly one span per moved key, each a child of the secondmate agent's carrier carrying `firstmate.backlog.item`, `firstmate.secondmate.id`, and `firstmate.route=local`, with the resource identifying the secondmate agent; a handoff whose receiver doorbell then fails keeps the landed key's span while still reporting the doorbell failure; and a refused non-canonical body, a dependency-stranding `tasks-axi mv` failure, and an untraced home session each post nothing.

The remote-handoff suite `tests/fm-remote-backlog-handoff.test.sh` (19 assertions) proves the same span on the remote route: a staged-but-undelivered outbox (dropped transfer after a successful put) posts nothing and keeps the item staged, while the retried delivery posts exactly one span after the durable remote receipt, a child of the parent-recorded agent carrier with `firstmate.route=remote`, and the outbox is released.

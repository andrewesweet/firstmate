...
Comparison base: branch `fm/fm-trace-attrs` at `76d44055` (plus this PR's own commits).
The assertions above in this file were re-run in this environment with the counts stated in each paragraph (30 unit, 15 spawn-path, 6 remote-route), including the resource-attribute export coverage: the fixed key list rendered from the task meta with W3C Baggage percent encoding, secondmate-id presence only for a secondmate, the enabled pane export ordered after `TRACEPARENT` and before the launch literal and matching the meta, the disabled path byte-identically sending nothing, the allowlist retention term, and the remote render carrying `firstmate.task.kind=secondmate` on the remote host with no new remote argument.

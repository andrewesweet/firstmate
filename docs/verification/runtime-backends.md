...
- Live run: `FM_BRANCH_MOD_LIVE=1 bin/fm-test-run.sh tests/fm-branch-claude-mod-live-e2e.test.sh` (one Claude Code 2.1.273 primary on `--model sonnet` in tmux, launched with the settings the owning page lists, supervising one stand-in task in a temporary scratch home, 143 s end to end) printed `ok - Claude Code 2.1.273 loads the supervision-branch mod enabled and main holds the session lock`, `ok - the first routine wake passes the classifier and spawns the branch agent, which reports it routine`, `ok - the captain-class wake is passed to main by the classifier with a covering outcome row, and the next routine wake reaches the same agent through SendMessage`, and `ok - one spawn, every send successful, no dropped hand-back, no backstop delivery across the run`.
  The module's event log for that run held three `classifier` records on `haiku` (`routine` in 908 ms, `captain` in 1,237 ms, `routine` in 1,079 ms), one `agent.spawn`, one `agent.send` whose text carried `"success":true,"message":"Resuming agent fm-branch"`, two `wake.delivered` (`"via":"spawn"` then `"via":"send"` to the same agent id), one `wake.passed` with `"why":"classifier captain"`, one `pass.cover` with `"processed":true`, one `report.call` (`routine`), two `turn.complete.main` (the lock turn and the drain of the passed `done:` wake), and no `deliver.captain`, `handback.dropped`, or `backstop.delivered`; the branch's first turn took 8.2 s over 4 steps at 15,637 tokens a step (46,480 cached and 16,059 cache-creation input tokens), and the second send was still in the branch's hands when the test ended.

### 2026-09-23 Claude Code 2.1.281 pin evidence

Evidence produced 2026-09-23 on Linux 6.6.87.2-microsoft-standard-WSL2 x86_64, Claude Code 2.1.281, Node v24.21.0, with `CLAUDE_CODE_PIN` in `hooks/branch.ts` at `2.1.281` and the plugin test suite importing its `PIN` from the module.
The changelog reading covered every release the pin had skipped: the upstream changelog carries no 2.1.279 entry (it goes straight from 2.1.278 to 2.1.280), and the 2.1.280 and 2.1.281 entries were read in full.
2.1.280 is dominated by the Opus 5.5 default-model switch, dialog and mouse-navigation fixes, and a `PermissionRequest` change (an agent-type hook no longer runs there, a surface the mod does not use), plus hook output sizes on the `hook_execution_complete` OpenTelemetry event.
2.1.281 adds MCP server checks to `claude plugin validate`, stops `validate` flagging listing metadata keys in plugin.json as unknown fields, improves plugin hook-failure errors to name the offending plugin, and fixes `--plugin-dir` on a folder of plugins that also has a `.claude-plugin/marketplace.json` loading one empty plugin - a layout the mod's single-plugin `--plugin-dir` path does not use.
Nothing announced touches the function-hooks module API, `$.tool.register`, `$.agent.spawn`, `$.prompt.submit`, the Stop hook contract, or the mod's `--plugin-dir` directory loading.

- Version and strict validation: `claude --version` printed `2.1.281 (Claude Code)`, and `claude plugin validate --strict .claude/mods/fm-branch-mod` printed `✔ Validation passed` (measured 877 ms) with the scan lines `./branch.ts hooks: session.start, turn.start, prompt.submit, tool.call, turn.step, session.compact, turn.complete`, `./branch.ts env writes: nothing`, and `./branch.ts env reads: CLAUDE_CODE_CHILD_SESSION, CLAUDE_CODE_FORCE_SESSION_PERSISTENCE, FM_CLASSIFY_CAPTAIN_HELD_VERB, FM_CLASSIFY_PAUSED_VERB, FM_CLASSIFY_RESERVED_KEY_PREFIXES, FM_CLASSIFY_RESOLVE_VERB, FM_CONFIG_OVERRIDE, FM_HOME, FM_ROOT_OVERRIDE, FM_STATE_OVERRIDE`.
  `bin/fm-test-run.sh tests/fm-branch-claude-mod-plugin.test.sh` printed `FM_TEST_BEGIN 2026-09-23T21:21:54Z tests/fm-branch-claude-mod-plugin.test.sh family=live-harness-optin expected_gate_skip=live-capability`, `ok - Claude Code 2.1.281 (Claude Code) validates the supervision-branch mod strictly: the documented hooks, the spawn, the classifier, and only the home-resolution, transcript-persistence, and fold-vocabulary environment`, `ok - Claude Code 2.1.281 (Claude Code) runs the supervision-branch mod's plugin test suite clean: pin refusal, classification records, captain hand-back, and routine spawn`, and `FM_TEST_END 2026-09-23T21:21:59Z tests/fm-branch-claude-mod-plugin.test.sh exit=0 duration_ms=4685 gate_skip=false`, measuring 4685 ms.
- Live run: `FM_BRANCH_MOD_LIVE=1 bin/fm-test-run.sh tests/fm-branch-claude-mod-live-e2e.test.sh` printed `FM_TEST_BEGIN 2026-09-23T21:22:24Z tests/fm-branch-claude-mod-live-e2e.test.sh family=live-harness-optin expected_gate_skip=live-capability`, `ok - Claude Code 2.1.281 loads the supervision-branch mod enabled and main holds the session lock`, `ok - the first routine wake passes the classifier and spawns the branch agent, which reports it routine`, `ok - the captain-class wake is passed to main by the classifier with a covering outcome row, and the next routine wake reaches the same agent through SendMessage`, `ok - a wake after the in-memory window reaches the same persisted agent: the send succeeds, no rotation`, `ok - one spawn, every send successful, no dropped hand-back, no backstop delivery across the run`, `ok - a minutes-later wake on an inherited CLAUDE_CODE_CHILD_SESSION rotates to a fresh agent (why=unresumable) and keeps the wake`, and `FM_TEST_END 2026-09-23T21:30:31Z tests/fm-branch-claude-mod-live-e2e.test.sh exit=0 duration_ms=486282 gate_skip=false`, measuring 486282 ms end to end with exit 0.
...
The 2.1.277 changelog was read in full; it is a bug-fix release, and no announced change touched the function-hooks module API, `$.tool.register`, `$.agent.spawn`, `$.prompt.submit`, the Stop hook contract, or `--plugin-dir` directory loading.

- Version and strict validation: `claude --version` printed `2.1.277 (Claude Code)`, and `CLAUDE_CODE_ENABLE_FUNCTION_HOOKS=1 claude plugin validate --strict .claude/mods/fm-branch-mod` printed `✔ Validation passed`.
  `bin/fm-test-run.sh tests/fm-branch-claude-mod-plugin.test.sh` printed `FM_TEST_BEGIN 2026-09-18T18:44:13Z tests/fm-branch-claude-mod-plugin.test.sh family=live-harness-optin expected_gate_skip=live-capability`, `ok - Claude Code 2.1.277 (Claude Code) validates the supervision-branch mod strictly: the documented hooks, the spawn, the classifier, and only the home-resolution environment`, `ok - Claude Code 2.1.277 (Claude Code) runs the supervision-branch mod's plugin test suite clean: pin refusal, classification records, captain hand-back, and routine spawn`, and `FM_TEST_END 2026-09-18T18:44:15Z tests/fm-branch-claude-mod-plugin.test.sh exit=0 duration_ms=2072 gate_skip=false`, measuring 2072 ms.
- Live run: `FM_BRANCH_MOD_LIVE=1 bin/fm-test-run.sh tests/fm-branch-claude-mod-live-e2e.test.sh` printed `FM_TEST_BEGIN 2026-09-18T18:44:23Z tests/fm-branch-claude-mod-live-e2e.test.sh family=live-harness-optin expected_gate_skip=live-capability`, `ok - Claude Code 2.1.277 loads the supervision-branch mod enabled and main holds the session lock`, `ok - the first routine wake passes the classifier and spawns the branch agent, which reports it routine`, `ok - the captain-class wake is passed to main by the classifier with a covering outcome row, and the next routine wake reaches the same agent through SendMessage`, and `ok - one spawn, every send successful, no dropped hand-back, no backstop delivery across the run`.
  The live run printed `FM_TEST_END 2026-09-18T18:46:29Z tests/fm-branch-claude-mod-live-e2e.test.sh exit=0 duration_ms=125952 gate_skip=false` and `live exit=0`, measuring 125952 ms end to end.

The live guard that refreshes this entry is `FM_BRANCH_MOD_LIVE=1 bin/fm-test-run.sh tests/fm-branch-claude-mod-live-e2e.test.sh`.

### 2026-09-18 Claude Code 2.1.276 pin evidence

Evidence produced 2026-09-18 on Linux 6.6.87.2-microsoft-standard-WSL2 x86_64, Claude Code 2.1.276, Node v24.18.0, with `CLAUDE_CODE_PIN` in `hooks/branch.ts` at `2.1.276` and the plugin test suite importing its `PIN` from the module.
Claude Code 2.1.275 was skipped and never pinned; the 2.1.275 and 2.1.276 changelogs were read in full, and no announced change touched the function-hooks module API, `$.tool.register`, `$.agent.spawn`, `$.prompt.submit`, the Stop hook contract, or `--plugin-dir` directory loading.

- Version and strict validation: `claude --version` printed `2.1.276 (Claude Code)`, and `CLAUDE_CODE_ENABLE_FUNCTION_HOOKS=1 claude plugin validate --strict .claude/mods/fm-branch-mod` printed `✔ Validation passed`.
  `bin/fm-test-run.sh tests/fm-branch-claude-mod-plugin.test.sh` printed `FM_TEST_BEGIN 2026-09-18T12:32:59Z tests/fm-branch-claude-mod-plugin.test.sh family=live-harness-optin expected_gate_skip=live-capability`, `ok - Claude Code 2.1.276 (Claude Code) validates the supervision-branch mod strictly: the documented hooks, the spawn, the classifier, and only the home-resolution environment`, `ok - Claude Code 2.1.276 (Claude Code) runs the supervision-branch mod's plugin test suite clean: pin refusal, classification records, captain hand-back, and routine spawn`, and `FM_TEST_END 2026-09-18T12:33:01Z tests/fm-branch-claude-mod-plugin.test.sh exit=0 duration_ms=2167 gate_skip=false`.
- Live run: `FM_BRANCH_MOD_LIVE=1 bin/fm-test-run.sh tests/fm-branch-claude-mod-live-e2e.test.sh` printed `FM_TEST_BEGIN 2026-09-18T12:33:09Z tests/fm-branch-claude-mod-live-e2e.test.sh family=live-harness-optin expected_gate_skip=live-capability`, `ok - Claude Code 2.1.276 loads the supervision-branch mod enabled and main holds the session lock`, `ok - the first routine wake passes the classifier and spawns the branch agent, which reports it routine`, `ok - the captain-class wake is passed to main by the classifier with a covering outcome row, and the next routine wake reaches the same agent through SendMessage`, and `ok - one spawn, every send successful, no dropped hand-back, no backstop delivery across the run`.
  The live run printed `FM_TEST_END 2026-09-18T12:35:25Z tests/fm-branch-claude-mod-live-e2e.test.sh exit=0 duration_ms=136035 gate_skip=false` and `live exit=0`, measuring 136035 ms end to end.

The live guard that refreshes this entry is `FM_BRANCH_MOD_LIVE=1 bin/fm-test-run.sh tests/fm-branch-claude-mod-live-e2e.test.sh`.

### 2026-09-17 Claude Code 2.1.274 pin evidence

Evidence produced 2026-09-17 on Linux 6.6.87.2-microsoft-standard-WSL2 x86_64, Claude Code 2.1.274, Node v24.18.0, with `CLAUDE_CODE_PIN` in `hooks/branch.ts` at `2.1.274` and the plugin test suite importing its `PIN` from the module:

- Strict validation: `claude plugin validate --strict .claude/mods/fm-branch-mod` printed `✔ Validation passed` with the scan lines `./branch.ts hooks: session.start, turn.start, prompt.submit, tool.call, turn.step, session.compact, turn.complete`, `./branch.ts env writes: nothing`, and `./branch.ts env reads: FM_CONFIG_OVERRIDE, FM_HOME, FM_ROOT_OVERRIDE, FM_STATE_OVERRIDE`.

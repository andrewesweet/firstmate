...
`lib/fm-branch-eligibility.ts` is the shared fold both TypeScript folds will adopt - the bash v8 rule plus the guards the ports carry, with bash's behaviour wherever bash has one - and its leg must be byte-equal to the bash fold on every fixture, drift cases included.
Bash contributes the fold truth alone because no bash-side eligible-row scan exists, and the test asserts today's documented drift rather than hiding it: Pi lacks the v8 terminal-close rule (a ship task's open `needs-decision` followed by `done:` stays decision-owned there while bash and the mod clear it), Pi refuses a symlinked status log for the whole scan and the mod reads through the link (holding the row when the target has an open decision) where bash's refusal names an empty fold, and a torn-epoch queue row is refused by the mod (which validates the epoch field) while Pi claims it. Each drift assertion retires once the drifting port adopts the rule the other folds apply; for the v8 row that is the terminal-close rule (`done:`/`failed:` closes every open decision of a ship or scout task) that `bin/fm-classify-lib.sh` and the mod already apply.

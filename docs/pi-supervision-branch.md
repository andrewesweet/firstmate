...
A ship or scout task whose log ends in `done:` or `failed:` has no open decisions left (the bash v8 terminal close), and an unreadable or symlinked status log reads as an empty fold rather than influencing routing.
...
4. Each probe that settles with another failure doubles the next cooldown, up to one hour.
...
Pi joins the classifier and shadow capabilities rather than staying Claude-only: both hosts run one shared core, the `lib/fm-branch-classifier.ts` and `lib/fm-branch-shadow.ts` modules, whose canonical copies live under the Claude mod's `lib/` with the repo's tracked `lib/` entries symlinking to them - the loader's realpath-containment rule forces that direction (see [claude-supervision-branch.md](claude-supervision-branch.md)).

On every accepted wake with eligible rows and no away-posture record, the extension classifies before any row is claimed: a bash-gathered evidence bundle from `bin/fm-wake-evidence.sh <task>` - the same offset ownership as the mod, `state/.<task>.classifier-offset`, removed by teardown - and, unless the shared deterministic verb route decides the wake first ([claude-supervision-branch.md](claude-supervision-branch.md) "Classifier" owns that route, one capability on both hosts), one `completeSimple` call on the model named by `config/classifier-model`, no thinking and `maxTokens` 200, over the wake's reason line and that bundle.
...
The gates scorer's evidence-sufficiency mode and its truth sidecar (`state/.branch-shadow-truth.jsonl`) are scorer-owned and host-agnostic, so a Pi home judges sufficiency and arms the same `fm-procevent-when.sh` sufficiency watches as a mod home (see [claude-supervision-branch.md](claude-supervision-branch.md) "Shadow advisory trial").

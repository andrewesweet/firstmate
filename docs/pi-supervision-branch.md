...
`tests/fm-branch-eligibility.test.sh` pins the three-fold equivalence of the wake-eligibility and open-decision classification: one fixture set of status logs, wake-queue rows, and task metas driven through the bash fold (`status_open_decisions`), the Pi extension's `scopeForUnreadWake`, and the mod's exported `scopeForUnreadWake` bound through its exported `bind` (both exports are behavior-neutral and exist for this test), asserting byte-identical normalised scope JSON wherever the folds agree.

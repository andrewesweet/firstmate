...
`lib/fm-branch-eligibility.ts` is the shared fold both TypeScript folds adopt - the bash v8 rule plus the guards the ports carry, with bash's behaviour wherever bash has one - and its leg must be byte-equal to the bash fold on every fixture, drift cases included.
...
Bash contributes the fold truth alone because no bash-side eligible-row scan exists, and one documented drift remains: the mod reads through a symlinked status log (holding the row when the target has an open decision) where bash's refusal names an empty fold, which the lib and the Pi extension take; that drift assertion retires when the mod fold adopts the rule the other folds apply.

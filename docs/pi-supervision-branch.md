...
Bash contributes the fold truth alone because no bash-side eligible-row scan exists, and the test asserts today's documented drift rather than hiding it: Pi lacks the v8 terminal-close rule (a ship task's open `needs-decision` followed by `done:` stays decision-owned there while bash and the mod clear it), Pi refuses a symlinked status log for the whole scan where bash's refusal names an empty fold and the mod reads through, and a torn-epoch queue row is refused by the mod (which validates the epoch field) while Pi claims it - stage A2's shared fold module is what retires those assertions.

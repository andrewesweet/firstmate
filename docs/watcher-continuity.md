...
A started watcher's stderr goes to a per-spawn `fm-watch-arm-stderr.*` side file in the temporary directory, outside the home it watches, rather than the arm's own stderr, so a caller capturing the arm through a pipe regains control at the budget exit instead of waiting on the still-running watcher.
That side file is also the arm's reason surface. On a nonzero close of its own watcher child the arm names the resource that disappeared under it - the home, the state directory, or the code root - judged only on evidence that survives the child's exit, and when the child printed no `watcher: FAILED` line of its own the arm appends the side file's tail to its own `watcher: FAILED - watcher cycle exited <code> without an actionable reason` line, so a dying diagnostic written after the state directory went away is still reported.

...
SH
  ...
  body=$(awk '
    ...
    END { print count }
  ' "$home/curl.log")
  [ "$body" = 3 ] || fail "expected the release to post the third span, got $body"
  body=$(awk '
    /^ARGS:/ { next }
    /^--BODY-END--$/ { exit }
    /4444444444444448/ { seen = 1 }
    seen && /^\{"resourceSpans/ { buf = $0 }
    END { printf "%s", buf }
  ' "$home/curl.log")
  jq -e '
    .resourceSpans[0].scopeSpans[0].spans[0] |

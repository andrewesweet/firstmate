...
test_local_only_fork_remote_allows() {
  ...
  # So is the Claude supervision-branch mod's classifier read offset.
  printf '5' > "$case_dir/state/.task-x1.classifier-offset"
  ...
  [ ! -e "$case_dir/state/.task-x1.classifier-offset" ] \
    || fail "fork-allow: teardown left the task's classifier offset behind"

...
EOF
  ...
  assert_contains "$out" "COMPACTED SKILLS - SUMMARIES ARE VOID" \
    "a compact re-emit did not print the compacted-skills block"
  assert_contains "$out" "harness-adapters" "the block omitted a recorded skill"
  assert_contains "$out" "ask-user-authority" "the block omitted a recorded skill"
  assert_contains "$out" "Re-load each one, at its next" \

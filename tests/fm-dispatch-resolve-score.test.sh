...
# spawned before its only resolver record, so it stays uncovered.
...
dispatch-resolve-score:
  ...
  floors:
    floor=0.3 clear_frac=1.0000 (6/6) precision_vs_spawn=0.5000 (1/2) precision_vs_label=0.5000 (1/2)
    floor=0.4 clear_frac=1.0000 (6/6) precision_vs_spawn=0.5000 (1/2) precision_vs_label=0.5000 (1/2)
    floor=0.5 clear_frac=0.8333 (5/6) precision_vs_spawn=0.5000 (1/2) precision_vs_label=0.5000 (1/2)
    floor=0.6 * clear_frac=0.8333 (5/6) precision_vs_spawn=0.5000 (1/2) precision_vs_label=0.5000 (1/2)
    floor=0.7 clear_frac=0.8333 (5/6) precision_vs_spawn=0.5000 (1/2) precision_vs_label=0.5000 (1/2)
    floor=0.8 clear_frac=0.6667 (4/6) precision_vs_spawn=0.5000 (1/2) precision_vs_label=0.5000 (1/2)
    floor=0.9 clear_frac=0.3333 (2/6) precision_vs_spawn=0.0000 (0/1) precision_vs_label=0.0000 (0/1)
EOF
)
...
# t1's label names r2's when text and agrees; t3's label names rule_2 while the
# pick was rule_1, so rule agreement reads 1/2.
...
assert_contains "$out" '    floor=0.6 * clear_frac=0.8333 (5/6) precision_vs_spawn=0.5000 (1/2)' "unlabeled floor line keeps spawn precision"
pass "unlabeled replay omits every label-derived metric"

# --- custom floors: only the requested floors print, still marked -------------
run out code --floors 0.9,0.5 --labels "$LABELS"
expect_code 0 "$code" "custom floors exit 0"
assert_equals '2' "$(grep -c '^    floor=' <<<"$out")" "custom floors print exactly the requested floors"
assert_contains "$out" '    floor=0.5 clear_frac=0.8333 (5/6)' "custom floors keep ascending order"
assert_contains "$out" '    floor=0.9 clear_frac=0.3333 (2/6)' "custom floor 0.9 matches the default run"
assert_not_contains "$out" ' *' "no live-floor mark when 0.6 is not requested"
pass "custom floor lists are honored and ordered"
...
assert_contains "$_out" '    floor=0.6 * clear_frac=0.8333 (5/6) precision_vs_spawn=n/a (0/0)' "floors still score the resolve log with n/a precision"
...
_out=$(FM_DISPATCH_RESOLVE_LOG="$RESOLVE_LOG" FM_DISPATCH_SPAWNS_LOG="$SPAWNS_LOG" "$TOOL" --floors 1.5 2> "$TMP_ROOT/stderr")
expect_code 2 "$?" "out-of-range floors exit 2"
assert_contains "$(cat "$TMP_ROOT/stderr")" 'malformed --floors' "malformed floors are named"

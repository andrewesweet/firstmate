...
for (const step of plan) {
  ...
  const makeDeps = () => ({
    ...
    resetStepCounter: () => {},
    runShadow: (args) => shadows.push(args),
  ...
  out.push({
    ...
    evidence,
    passedWrites,
    shadows: shadows.map((s) => s.wakeKey),
...
# The shadow advisory fires detached on a routine grant with evidence.
if [ "$(step routine-grant | jq -c '.shadows')" = '["103:4"]' ]; then
  pass "a granted routine wake with evidence fires the shadow advisory with the wake key"
else
  fail "the shadow advisory trigger drifted: $(step routine-grant | jq -c '.shadows')"
fi

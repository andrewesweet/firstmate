...
// report-sequence, provider-latch, classifier, and shadow modules. This
...
// call, exactly as classifierDeps($)/shadowDeps($) always have.
...
function routerDeps($: any) {
  return {
    ...
    runShadow: (args: Parameters<typeof runShadowAdvisory>[1]) => {
      void runShadowAdvisory($, args)
    },
  ...
  return {
    ...
    hasOpenCall: async (task: string) => {
      try {
        ...
        return hasOpenNeedsDecision(lines, await statusKindOf(scopeScanDeps($), state, task), await foldVocabulary($))
      } catch {
        return false
      }
    },

...
// the outcome store, delivery, the status note, the shadow advisory, and
...
export interface WakeRouterDeps {
  ...
  runShadow(args: { wake: string; seqs: string[]; wakeKey: string; tasks: string[]; evidence: RoutedClassifierResult['evidence']; wakeNo: number }): void
...
export function createWakeRouter() {
  ...
  async function routeWake(deps: WakeRouterDeps, wakeText: string, source: string): Promise<'dropped' | 'passed'> {
    ...
    // the shadow advisory receives them in memory only.
    ...
    // Shadow advisory trial: detached and bounded, never delaying or
    // altering the delivery below. Any failure is recorded as unavailable
    // by the trial itself; nothing here may change the wake.
    if (c.evidence.length > 0) deps.runShadow({ wake: reason, seqs: scope.eligibleSeqs, wakeKey: scope.eligibleWakeKey, tasks: scope.eligibleTasks, evidence: c.evidence, wakeNo })

...
config/claude-function-hooks  optional presence flag opting every Claude worker launch from this home into Claude Code's function-hooks surface (launch environment carries CLAUDE_CODE_ENABLE_FUNCTION_HOOKS=1; content ignored); LOCAL, gitignored; inherited by secondmate homes; see docs/configuration.md "Claude function hooks"
...
data/                personal fleet records; LOCAL, gitignored as a whole
  ...
  spend-ledger.jsonl  append-only closed-task model-spend ledger, one best-effort schema-versioned line per closed ship or scout task; bin/fm-spend-query.sh owns the schema and answers standalone (docs/configuration.md "Task spend ledger")
...
state/               runtime records and signals; gitignored
  ...
  .wake-trace.<task>   telemetry-only copy of a presented task's trace carrier so its firstmate.wake span survives teardown until acknowledgement discards it; never queue authority; bin/fm-wake-drain.sh owns it
  ...
  .branch-mod-counters .branch-mod-passed branch-mod-events.jsonl branch-mod-classifications.jsonl branch-mod-shadow.jsonl  that mod's session counters keyed by the lock pid, its event log, and its durable records for the classifier and shadow trial: the queue rows passed to main and not yet acknowledged (`.branch-mod-passed`, shared with the Pi extension), the one-record-per-classification log read by bin/fm-branch-classifier-score.sh (`branch-mod-classifications.jsonl`, shared with the Pi extension), and the size-capped shadow advisory log read by bin/fm-branch-shadow-score.sh and bin/fm-branch-shadow-gates.sh (`branch-mod-shadow.jsonl`, shared with the Pi extension); module-owned, never touch. The gates scorer separately owns `state/.branch-shadow-truth.jsonl`, its durable per-wake snapshot of the decaying merge-poll, stale-repair and backstop-surfacing truth (docs/claude-supervision-branch.md "Shadow advisory trial")
  ...
  .compact-skills    skills this session had loaded before a Claude compaction, one per line; written by the PreCompact hook bin/fm-precompact-skills.sh, printed by bin/fm-session-start.sh's compact-source digest, overwritten by each later compaction's cumulative transcript
  ...
  retro-trigger/     durable retrospective-trigger store: generation, receipts/<generation>/, open-row marker, archive/<retro-id>/, and its lock; written only by bin/fm-retro-trigger.sh through its two producers (the fm-teardown.sh backlog-close hook and the fm-wake-lib.sh presentation annotations); present only when config/retro-cadence opts this home in; see docs/configuration.md "Retro cadence trigger"
...
On this fork the worker starts validation itself, so a bare no-mistakes `done [at=<epoch>]: {summary}` has no legitimate producer and is gated like every other ship done; only the checks-green report, or a `forge=gerrit` project's published-for-review report, is a ready signal.

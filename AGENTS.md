...
state/               runtime records and signals; gitignored
  ...
  .branch-mod-mode .branch-mod-classifier .branch-mod-index-note  Claude Code supervision-branch mod switches: the opt-in (`drop`) whose presence alone switches every bin/ piece the mod relies on, the classifier-ahead-of-the-branch flag, and the index-note-on-every-wake flag; LOCAL, operator-written; docs/claude-supervision-branch.md
  .branch-mod-counters branch-mod-events.jsonl branch-mod-classifications.jsonl  that mod's session counters keyed by the lock pid, its size-capped append-only event log, and its durable one-record-per-call classification log read by bin/fm-branch-classifier-score.sh; module-owned, never touch
  ...
  .compact-skills    the Claude PreCompact hook's record of the skills this session had loaded before a compaction voided their bodies; bin/fm-precompact-skills.sh owns the format, and the compact-source digest prints it

...
On this fork the worker starts validation itself, so a bare no-mistakes `done [at=<epoch>]: {summary}` has no legitimate producer and is gated like every other ship done; only the checks-green report is a ready signal.

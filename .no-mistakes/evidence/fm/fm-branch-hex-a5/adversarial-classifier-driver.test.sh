...
# Adversarial live driver (evidence only, not tracked): a THROWING classifier
# completion (failed call) must pass rows to main exactly like a captain
# verdict; config/classifier-model overrides the recorded model; with
# config/classifier-shadow unset a routine wake runs no shadow trial.
test_adversarial_failed_call_passes_and_shadow_stays_off() {
  local repo home out status
  repo="$TMP_ROOT/adv-root"
  home="$TMP_ROOT/adv-home"
  mkdir -p "$home/state" "$home/config"
  install_pi_branch_extension_fixture "$repo"
  printf 'gpt-5-nano\n' > "$home/config/classifier-model"
  PLUGIN="$repo/.pi/extensions/fm-branch-supervision.ts" FM_HOME="$home" FM_ROOT_OVERRIDE="$ROOT" \
    DRIVER_PRELUDE="$DRIVER_PRELUDE" node --input-type=module > "$TMP_ROOT/node-output" 2>&1 <<'EOF'
const prelude = process.env.DRIVER_PRELUDE;
await eval(`(async () => { ${prelude}; globalThis.__t = { pi, fire, dispatch, settle, defaultSessionCtx, home, makeOffer, bus }; })()`);
...
writeFileSync(`${home}/state/.lock`, `${process.ppid}\n`);
await fire("session_start", {}, defaultSessionCtx);
const rows = (f) => { try { return readFileSync(f, "utf8").trim().split("\n").filter(Boolean).map((l) => JSON.parse(l)); } catch { return []; } };
const classFile = `${home}/state/branch-mod-classifications.jsonl`;
const passedFile = `${home}/state/.branch-mod-passed`;

// 1. failed call
globalThis.__fmClassifierAnswer = async () => { throw new Error("provider exploded"); };
const offer = dispatch("signal: task-9 done: PR https://example.com/pr/9 checks green");
let rejected = "";
try { await offer.settlement; } catch (e) { rejected = String(e.message); }
console.log("settlement rejection:", rejected);
if (!rejected.includes("classifier routed the wake to main: uncertain")) throw new Error("failed call did not pass to main as uncertain");
const rec = rows(classFile);
console.log("classification record:", JSON.stringify(rec[0]));
if (rec.length !== 1 || rec[0].verdict !== "uncertain") throw new Error("bad record");
if (rec[0].model !== "gpt-5-nano") throw new Error("config/classifier-model not honoured");
if (!String(rec[0].reason).includes("provider exploded")) throw new Error("failure text not recorded");
console.log("passed guard:", readFileSync(passedFile, "utf8"));
if (JSON.stringify(JSON.parse(readFileSync(passedFile, "utf8"))) !== '["1"]') throw new Error("guard wrong");
let cover = null;
await settle(() => { const r = rows(`${home}/state/branch-outcomes.jsonl`); cover = r.find((x) => x.verdict === "captain") ?? null; return cover !== null; }, "cover row");
console.log("cover row:", JSON.stringify(cover));
if (!cover.summary.startsWith("Passed to main directly (classifier uncertain): ")) throw new Error("cover summary wrong");
if ((globalThis.__fmPrompts ?? []).length !== 0) throw new Error("branch was prompted despite pass");
if (existsSync(`${home}/state/branch-mod-shadow.jsonl`)) throw new Error("shadow ran on a passed wake");

// 2. routine wake, shadow unset -> branch prompted, no shadow log
writeFileSync(`${home}/state/.wake-queue`, "1\t2\tsignal\tbranch-driver.status\tsignal: next wake\n");
globalThis.__fmClassifierAnswer = async () => JSON.stringify({ verdict: "routine", reason: "healthy" });
const second = makeOffer("signal: next wake");
bus.emit("fm-branch-supervision:dispatch", second);
if (!second.accepted) throw new Error("routine wake not accepted");
try { await second.settlement; } catch {}
await settle(() => (globalThis.__fmPrompts ?? []).length === 1, "branch prompt");
console.log("branch prompted once for routine wake; classifier calls:", globalThis.__fmClassifierCalls.length, "maxTokens:", globalThis.__fmClassifierCalls[1].maxTokens);
if (globalThis.__fmClassifierCalls[1].maxTokens !== 200) throw new Error("maxTokens cap not 200");
await new Promise((r) => setTimeout(r, 1500));
if (existsSync(`${home}/state/branch-mod-shadow.jsonl`)) throw new Error("shadow ran with config unset");
console.log("no branch-mod-shadow.jsonl with config/classifier-shadow unset");
process.exit(0);
EOF
  status=$?
  out=$(cat "$TMP_ROOT/node-output")
  expect_code 0 "$status" "adversarial driver must hold: $out"
  printf '%s\n' "$out"
  pass "a failed classifier call passes rows to main; classifier-model honoured; shadow off when unset"
}
test_adversarial_failed_call_passes_and_shadow_stays_off

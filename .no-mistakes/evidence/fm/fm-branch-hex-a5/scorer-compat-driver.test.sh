...
# Evidence-only driver: Pi-produced classification and shadow logs feed the
# REAL scorers (bin/fm-branch-classifier-score.sh, bin/fm-branch-shadow-score.sh,
# bin/fm-branch-shadow-gates.sh) unchanged.
test_pi_logs_feed_real_scorers() {
  local repo home out status
  repo="$TMP_ROOT/scorer-root"; home="$TMP_ROOT/scorer-home"
  mkdir -p "$home/state" "$home/config"
  install_pi_branch_extension_fixture "$repo"
  printf 'jev\n' > "$home/config/classifier-shadow"
  PLUGIN="$repo/.pi/extensions/fm-branch-supervision.ts" FM_HOME="$home" FM_ROOT_OVERRIDE="$ROOT" \
...
const prelude = process.env.DRIVER_PRELUDE;
await eval(`(async () => { ${prelude}; globalThis.__t = { pi, fire, dispatch, settle, defaultSessionCtx, home }; })()`);
const { fire, dispatch, settle, defaultSessionCtx, home } = globalThis.__t;
import { readFileSync, writeFileSync } from "node:fs";
writeFileSync(`${home}/state/.lock`, `${process.ppid}\n`);
await fire("session_start", {}, defaultSessionCtx);
...
globalThis.__fmClassifierAnswer = async () => JSON.stringify({ verdict: "routine", reason: "healthy" });
const offer = dispatch("signal: routine wake");
try { await offer.settlement; } catch {}
await settle(() => rows(`${home}/state/branch-mod-shadow.jsonl`).length >= 4, "shadow records");
process.exit(0);
EOF
  status=$?; out=$(cat "$TMP_ROOT/node-output")
  expect_code 0 "$status" "driver must hold: $out"
  echo "== state/branch-mod-classifications.jsonl (Pi-produced)"; cat "$home/state/branch-mod-classifications.jsonl"
  echo "== state/branch-mod-shadow.jsonl (Pi-produced, first record)"; head -1 "$home/state/branch-mod-shadow.jsonl"
  echo "== bin/fm-branch-classifier-score.sh -v"; FM_HOME="$home" "$ROOT/bin/fm-branch-classifier-score.sh" -v "$home/state/branch-mod-classifications.jsonl"; echo "exit=$?"
  echo "== bin/fm-branch-shadow-score.sh -v"; FM_HOME="$home" "$ROOT/bin/fm-branch-shadow-score.sh" -v "$home/state/branch-mod-shadow.jsonl" "$home/state/branch-outcomes.jsonl"; echo "exit=$?"
  echo "== bin/fm-branch-shadow-gates.sh"; FM_HOME="$home" "$ROOT/bin/fm-branch-shadow-gates.sh" "$home/state/branch-mod-shadow.jsonl" "$home/state/branch-outcomes.jsonl" 2>&1; echo "exit=$?"
  pass "Pi-produced logs are read by the real scorers"
}
test_pi_logs_feed_real_scorers

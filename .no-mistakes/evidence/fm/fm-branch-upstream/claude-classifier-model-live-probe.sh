...
# ---- probe: classifier model resolution on the Claude Code host --------------
EVIDENCE=${EVIDENCE:?}
run_lab() { # <label> <classifier-model config or -> <socket>
  local label=$1 cfg=$2 socket=$3
  make_lab "$socket"; eval "$4=\$LAB"
  [ "$cfg" != - ] && printf '%s\n' "$cfg" > "$HOME_DIR/config/classifier-model"
  start_claude_session
  wait_event session.start '"enabled":true' 'the module loading enabled'
  wait_composer
  take_lock
  start_dummy
  wait_event classifier '"kind":"classifier"' "the classifier's verdict on the working line"
  sleep 3
  {
    echo "# lab=$label classifier-model=$cfg supervision-branch-model=<absent>"
    events classifier
  } | tee -a "$EVIDENCE/claude-live-classifier-events.jsonl"
  cp "$STATE/branch-mod-classifications.jsonl" "$EVIDENCE/claude-live-$label-classifications.jsonl" 2>/dev/null || true
  teardown_lab "$socket" "$LAB"
}
run_lab explicit-sonnet sonnet "$SOCKET1" LAB1
run_lab unresolvable-name no-such-model-fm-probe "$SOCKET2" LAB2
pass "probe complete"

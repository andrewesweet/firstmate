...
. "/home/andre/.no-mistakes/worktrees/feb37d45d9da/01M2X8XJ3FRFGA2KTF52ANQEF3/tests/lib.sh"
...
printf 'jev\n' > "$HOME_DIR/config/classifier-shadow"
printf 'TYPESAFE_API_KEY=sk-lab-fake-key\n' > "$HOME_DIR/.env"
mkdir -p "$LAB/jev-calls"
cat > "$SHIM/curl" <<EOF
#!/usr/bin/env bash
# lab stand-in for curl: records how the jev helper invoked it, answers 200 with a canned Jev reply
n=\$(date +%s%N); d='$LAB/jev-calls'
printf '%s\n' "\$*" > "\$d/\$n.argv"
if [ -e /dev/fd/3 ]; then cat <&3 > "\$d/\$n.fd3"; fi
env | grep -c '^TYPESAFE_API_KEY=' > "\$d/\$n.envleak" || true
cat > "\$d/\$n.body"
out=''; prev=''
for a in "\$@"; do [ "\$prev" = -o ] && out=\$a; prev=\$a; done
printf '%s' '{"model":"jev-1.13.0","answers":{"route":{"type":"choice","choice":"routine","confidence":0.91},"phase":{"type":"choice","choice":"working","confidence":0.88},"severity":{"type":"score","score":1,"confidence":0.8},"no_new_outcome":{"type":"noul","noul":0.1}}}' > "\$out"
printf '200'
EOF
chmod +x "$SHIM/curl"
...
# --- shadow trial evidence from the real session ------------------------------
EVD='/home/andre/.no-mistakes/evidence/01M2X8XJ3FRFGA2KTF52ANQEF3'
cp "$STATE/branch-mod-shadow.jsonl" "$EVD/live-branch-mod-shadow.jsonl" 2>/dev/null || echo "# no shadow log written"
cp "$STATE/branch-outcomes.jsonl" "$EVD/live-branch-outcomes.jsonl" 2>/dev/null || true
cp "$STATE/branch-mod-events.jsonl" "$EVD/live-branch-mod-events.jsonl" 2>/dev/null || true
cp "$STATE/.wake-queue" "$EVD/live-wake-queue.tsv" 2>/dev/null || true
mkdir -p "$EVD/live-jev-calls"; cp "$LAB/jev-calls"/* "$EVD/live-jev-calls/" 2>/dev/null || true
FM_HOME="$HOME_DIR" FM_STATE_OVERRIDE="$STATE" FM_CONFIG_OVERRIDE="$HOME_DIR/config" "$ROOT/bin/fm-branch-shadow-score.sh" -v > "$EVD/live-shadow-score.txt" 2>&1 || true
echo "# evidence copied to $EVD"
teardown_lab "$SOCKET" "$LAB"
